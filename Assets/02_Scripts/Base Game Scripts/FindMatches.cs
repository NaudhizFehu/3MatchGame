...
public class FindMatches : MonoBehaviour
{
            ...
            for(int j = _row - 1; j <= _row + 1; j++)
            {
                if(i >= 0 && i < board.width && j >= 0 && j < board.height)
                {
                    if(board.allBeads[i, j] != null)
                    {
                        ...
                        if (bead.isColumnBomb && !bead.isMatched)
                        {
                            beads.Union(GetColumnPieces(i)).ToList();
                        }
                        if (bead.isRowBomb && !bead.isMatched)
                        {
    ...
    public void CheckBombs(MatchType _matchType)
    {
        //Did the player move something?
        if(board.currentBead != null)
        {
            //Is the piece they moved matched?
            if(board.currentBead.isMatched && board.currentBead.tag == _matchType.color)
            {
                //make it unmatched
            ...
            else if(board.currentBead.otherBead != null)
            {
                ...
                if(otherBead.isMatched && otherBead.tag == _matchType.color)
                {
                    //Make it unmatched
